...
OUTPUT_PATH = args.IMAGE_PATH[0:args.IMAGE_PATH.rfind(
    ".")]+"_solved"+args.IMAGE_PATH[args.IMAGE_PATH.rfind("."):]
...
for cnt in contours:
    ...
    if rect_size <= 2000 and rect_size > 400:
        ...
        config = ("--oem 2 --psm 10")
        detected_digit = pytesseract.image_to_string(
            blurred, lang="eng", config=config)
...
for x in range(0, 9):
    for y in range(0, 9):
        ...
        cv2.putText(original_image, solved_sudoku[row+col],
                    (pos_x, pos_y), font, 1.5, (0, 0, 0), 1, cv2.LINE_AA)
